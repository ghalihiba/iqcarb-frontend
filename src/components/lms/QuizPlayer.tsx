...
// ─── Composant principal ──────────────────────────────────────
export default function QuizPlayer({ id_cours, onComplete, onMaxAttemptsReached }: QuizPlayerProps) {
  ...
  const handleSubmit = useCallback(async () => {
    if (!quiz || failedAttempts >= MAX_FAILED_ATTEMPTS) return;
    setSubmitting(true);
    try {
  ...
  }, [quiz, answers, onComplete, failedAttempts]);
  ...
  if (resultat) {
    ...
    const attemptsReached = failedAttempts >= MAX_FAILED_ATTEMPTS && !resultat.passed;
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-8 text-center">
          ...
          {!resultat.passed && (
            <p className={`text-xs font-semibold mb-6 ${
              attemptsReached ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'
            }`}>
              Tentatives échouées : {failedAttempts}/{MAX_FAILED_ATTEMPTS}
            </p>
          )}

          {/* Boutons */}
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {!resultat.passed && !attemptsReached && (
              <button
                onClick={handleRetry}
            ...
            {!resultat.passed && attemptsReached && (
              <button
                ...
                className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold bg-red-600 hover:bg-red-700 text-white"
              >
                <RotateCcw className="w-4 h-4" />
                Recommencer tout le module
              </button>
            )}
            {resultat.passed ? (
              <button
                onClick={onComplete}
                className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-colors bg-emerald-600 hover:bg-emerald-700 text-white"
              >
              ...
              <button
                disabled
                className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-colors bg-gray-100 dark:bg-gray-700 text-gray-400 cursor-not-allowed"
              >
                <ChevronRight className="w-4 h-4" />
                {attemptsReached ? 'Limite atteinte' : 'Réussissez le quiz pour continuer'}
              </button>
            )}
          </div>
        </div>
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3.5 rounded-xl border-2 text-sm transition-all duration-150 ${
                  isSelected
                    ...
                    : 'border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:border-green-300 dark:hover:border-green-700 hover:bg-green-50/50 dark:hover:bg-green-900/10'
                }`}
              >
