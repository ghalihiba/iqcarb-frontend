...
import {
  ...
  CreditCard, Briefcase, PanelLeftClose, PanelLeftOpen,
} from 'lucide-react';
...
import SimulateurCO2Pedagogique from '@/pages/SimulateurCO2Pedagogique';
...
function parseMarkdown(text: string): string {
  ...
  r = r.replace(/^\|(.+)\|$/gm, (_, cells) => {
    const tds = cells.split('|').map((c: string) =>
      `<td style="padding:8px 12px;border:1px solid var(--iq-border);font-size:13px;color:var(--iq-text-1)">${c.trim()}</td>`
    ).join('');
    return `<tr>${tds}</tr>`;
  });
  r = r.replace(/((<tr>.*<\/tr>\n?)+)/gs,
  ...
  r = r.replace(/^---$/gm, '<hr style="border:none;border-top:1px solid var(--iq-border);margin:24px 0"/>');
  r = r.replace(/^>\s*(.+)$/gm,
    '<blockquote style="border-left:4px solid #10b981;padding:12px 16px;margin:16px 0;background:rgba(16,185,129,0.08);border-radius:0 8px 8px 0;color:var(--iq-text-1);font-style:italic">$1</blockquote>'
  );
  r = r.replace(/^### (.+)$/gm,
    '<h3 style="font-size:15px;font-weight:700;color:var(--iq-text-1);margin:20px 0 8px">$1</h3>'
  );
  r = r.replace(/^## (.+)$/gm,
    '<h2 style="font-size:18px;font-weight:700;color:var(--iq-text-1);margin:28px 0 12px;padding-bottom:8px;border-bottom:1px solid var(--iq-border)">$1</h2>'
  );
  r = r.replace(/^- (.+)$/gm,
    '<li style="display:flex;gap:8px;margin:4px 0;color:var(--iq-text-1);font-size:14px"><span style="color:#10b981">●</span><span>$1</span></li>'
  );
  r = r.replace(/(<li[^>]*>.*?<\/li>\n?)+/gs,
    '<ul style="list-style:none;padding:0;margin:12px 0">$&</ul>'
  );
  r = r.replace(/\*\*(.+?)\*\*/g, '<strong style="font-weight:700;color:var(--iq-text-1)">$1</strong>');
  r = r.replace(/\*(.+?)\*/g, '<em style="font-style:italic">$1</em>');
  r = r.replace(/^(?!<[hublpdt]|<\/|<strong|<em|<blockquote)(.+)$/gm,
    '<p style="color:var(--iq-text-1);font-size:14px;line-height:1.7;margin:8px 0">$1</p>'
  );
    ...
    <div className="max-w-3xl">
      {videoId ? (
        ...
        <div className="bg-gray-100 dark:bg-gray-700 rounded-2xl p-8 text-center">
          <a href={url} target="_blank" rel="noopener noreferrer" className="text-green-600 underline text-sm">{url}</a>
        </div>
      )}
      <div className="mt-6 flex justify-end">
        <button onClick={onComplete} disabled={isCompleted}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold ${
            isCompleted ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 cursor-default' : 'bg-green-600 hover:bg-green-700 text-white'
          }`}>
          <CheckCircle className="w-4 h-4" />
...
function TypeBadge({ type }: { type: string }) {
  const map: Record<string, { label: string; icon: React.ElementType; cls: string }> = {
    ...
    VIDEO:      { label: 'Vidéo',       icon: Video,      cls: 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' },
    QUIZ:       { label: 'Quiz',        icon: HelpCircle, cls: 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' },
    ...
    ETUDE_DE_CAS:  { label: 'Cas pratique', icon: Briefcase, cls: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300' },
  };
...
export default function CoursDetail() {
  ...
  const [failedCompletionIds, setFailedCompletionIds] = useState<Set<string>>(new Set());
    ...
    const load = async () => {
      try {
        ...
        modulesAvecCours.forEach((m) => {
          (m.cours ?? []).forEach((c) => {
            if (c.est_complete && !failedCompletionIds.has(c.id_cours)) completed.add(c.id_cours);
          });
        });
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      if (cours?.type_contenu === 'QUIZ' && parcours?.modules) {
        const currentModule = parcours.modules.find((m) =>
          (m.cours ?? []).some((c) => c.id_cours === id)
        );
        if (currentModule) {
          const moduleCoursIds = (currentModule.cours ?? []).map((c) => c.id_cours);
          setFailedCompletionIds((prev) => {
            const next = new Set(prev);
            moduleCoursIds.forEach((courseId) => next.delete(courseId));
            return next;
          });
        }
      }
  ...
  }, [id, completedIds, cours?.type_contenu, parcours, toast]);

  const handleQuizMaxAttemptsReached = useCallback(() => {
    if (!id || !parcours?.modules) return;
    const currentModule = parcours.modules.find((m) => (m.cours ?? []).some((c) => c.id_cours === id));
    if (!currentModule) return;

    const moduleCoursIds = (currentModule.cours ?? []).map((c) => c.id_cours);
    setFailedCompletionIds((prev) => new Set([...prev, ...moduleCoursIds]));
    ...
    const firstCours = (currentModule.cours ?? [])[0];
    toast.error('Quiz non validé', '3 tentatives échouées. Le module est remis non lu.');
    if (firstCours?.id_cours) {
      navigate(`/lms/cours/${firstCours.id_cours}`);
    }
  }, [id, navigate, parcours, toast]);
  ...
  const renderContent = () => {
    ...
    const coursWithObjectif = cours as Cours & { objectif_pct?: number | null };
    const isCompleted = completedIds.has(id ?? '');
    const type    = cours.type_contenu ?? 'ARTICLE';
    const contenu = (cours as Cours & { contenu?: string }).contenu ?? cours.contenu_url ?? '';

    if (coursWithObjectif.type_contenu === 'ETUDE_DE_CAS') {
      return (
        <SimulateurCO2Pedagogique
      ...
      if (type === 'QUIZ') {
        return (
          <QuizPlayer
            ...
            onMaxAttemptsReached={handleQuizMaxAttemptsReached}
          />
        );
      }
  ...
  return (
    <div className="iq-shell">
      ...
      <div className="iq-main iq-dotgrid relative flex overflow-hidden">

        {/* Contenu principal */}
        <main className="flex-1 overflow-y-auto">

          {/* Header sticky */}
          <div className="iq-page-header px-8 py-4 flex items-center justify-between sticky top-0 z-10">
